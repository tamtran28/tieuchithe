...
# =========================================================
with tab1:
    st.header("📌 Nhập file xử lý 1.3.2 – Thẻ")
    ...
    for key, label in file_list.items():
        uploaded_files_the[key] = st.file_uploader(f"Upload file {label}", type=["xlsx","xls"], key=key)

    if st.button("▶ Chạy xử lý 1.3.2"):
        ...
        if missing:
            ...
            st.dataframe(df_the_result.head())

            st.session_state["df_the_result"] = df_the_result
...
# =========================================================
with tab2:
    st.header("📌 Nhập file xử lý POS – 6.2")
    
    df_pos_raw = st.file_uploader("Upload file POS (MUC51_1600)", type=["xlsx","xls"])
    df_trans_raw = st.file_uploader("Upload file giao dịch POS (6.2a)", type=["xlsx","xls","csv"])

    colA, colB = st.columns(2)
    with colA:
        start_date = st.date_input("Ngày bắt đầu thời hiệu kiểm toán")
    with colB:
        end_date = st.date_input("Ngày kết thúc thời hiệu kiểm toán")

    if st.button("▶ Chạy xử lý POS"):
        if df_pos_raw is None or df_trans_raw is None:
            st.error("Thiếu file POS hoặc giao dịch!")
        else:
            df_pos_raw = pd.read_excel(df_pos_raw)
            df_trans_raw = pd.read_excel(df_trans_raw)
            ...
            st.dataframe(df_pos_result.head())

            st.session_state["df_pos_result"] = df_pos_result
...
with tab3:
    ...
    if "df_the_result" in st.session_state:
        buf = io.BytesIO()
    ...
    if "df_pos_result" in st.session_state:
        buf2 = io.BytesIO()
        st.session_state["df_pos_result"].to_excel(buf2, index=False)
        st.download_button(
            "⬇ Tải file kết quả POS (6.2)",
            data=buf2.getvalue(),
            file_name="ket_qua_pos.xlsx"
        )
